...
public class ColorPreference extends Preference implements ColorPickerSwatch
    ...
    private int mTitle = stringTitle(getContext());
    ...
    public static int stringTitle(Context context) {
        return context.getResources().getIdentifier("color_picker_default_title", "string", context.getPackageName());
    }

    public ColorPreference(Context context, AttributeSet attrs) {
        super(context, attrs);

        int id = (ColorPickerDialog.getID("default_rainbow","array", context));
        if (id != 0) {
            mColors = getContext().getResources().getIntArray(id);
        }
        id = 0;
        if (id != 0) {
            mTitle = (ColorPickerDialog.getID("color_picker_default_title","string",context));
        }
        mColumns = 5;
        mMaterial = true;
...
/*
        TypedArray a = context.getTheme().obtainStyledAttributes(attrs, R.styleable
                .ColorPreference, 0, 0);
        try {
            int id = a.getResourceId(R.styleable.ColorPreference_colours, ColorPickerDialog.getID("rainbow","array"));
            if (id != 0) {
                mColors = getContext().getResources().getIntArray(id);
            }
